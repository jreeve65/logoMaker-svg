...
describe('Square',()=> {
    describe('Square Constructed',()=>{
        it('shoud be an instance of Square',()=>{
            const sqr = new Square();
            expect(sqr).toBeInstanceOf(Square);
        });
    });
    describe('renderSquare',()=>{
        it('should return a template literal for svg',()=>{
            const sqr = new Square('square','green','BAY','gold')
            expect(sqr.renderSquare()).toEqual(`${renderSVG()}<rect width="150" height="150" x="10" y="10" rx="20" ry="20" fill="green" /><text x="85" y="105" font-size="55" text-anchor="middle" fill="gold">BAY</text></svg>`)
        });
    });
});
});
